function stab = is_stable(self)
    ...
    if ~isempty(self.spatial_xyz) || (isempty(self.gab) && isempty(self.gabcd))
        L2=((1-1i*stab_w/self.alpha(1)).*(1-1i*stab_w/self.beta(1))).^-2; 
        zprime = self.xyz(3)*(self.alpha(1)+self.beta(1)).^2./(self.alpha(1)*self.beta(1));       
        d = (( (1-1i*stab_w/self.gammae).^2) - self.xyz(1)).*(1+L2.*zprime) - self.xyz(2).*(1+zprime).*exp(1i*stab_w*self.t0);
